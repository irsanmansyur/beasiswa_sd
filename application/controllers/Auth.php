...
class Auth extends CI_Controller
{
	...
	private function _login()
	{
		...
		if ($user) {
			...
			if ($user['is_active'] == 1) {
				...
				$this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">Email anda telah di Non Aktifkan!</div>');
				redirect('auth');
			}
		} else {
	...
	public function registration()
	{
		...
		$this->form_validation->set_rules('tanggal_lahir', 'required|trim');

		$this->form_validation->set_rules('jenis_kelamin', 'required|trim');
		$this->form_validation->set_rules('email', 'Email', 'required|trim|valid_email|is_unique[user.email]', [
			'is_unique' => 'Email ini sudah digunakan!'
		]);
		$this->form_validation->set_rules('alamat', 'required|trim');
		$this->form_validation->set_rules('no_tlp', 'required|trim');
